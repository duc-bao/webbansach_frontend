...
interface SelectMultipleProps {
	values: CategoryModel[]; // Giá trị để in ra dữ liệu select
...
export const SelectMultiple: React.FC<SelectMultipleProps> = (props) => {
	const handleChange = (
		...
		const dataSelectedId = dataSelected.map((i) => i.idCategory);
			...
			<FormControl sx={{ mb: 3, width: "100%" }} size='small'>
				...
				>
					{props.values.map((genre) => (
						<MenuItem key={genre.idCategory} value={genre.nameCategory}>
							<Checkbox
								checked={
									props.selectedListName.indexOf(genre.nameCategory) > -1
								}
							/>
							<ListItemText primary={genre.nameCategory} />
						</MenuItem>
					))}
				</Select>
			</FormControl>
